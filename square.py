...
class Square:
    ...
    def check_proximity(self, xy):
        ...
        if self.position == xy:
            return False
        if (abs(xy[0] - self.position[0]) <= 1 and xy[1] == self.position[1]) or (
                abs(xy[1] - self.position[1]) <= 1 and xy[0] == self.position[0]):
            return True
        return False
    ...
    def equals(self, square):
        if self.position == square.position:
            if self.label == square.label:
                return True
            else:
                return False
        else:
